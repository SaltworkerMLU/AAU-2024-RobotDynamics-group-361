...
for t = linspace(0, animation_time, num_frames)
    ...
    H12 = m(2)*l(1)*c(2)*cos(theta{2}(i)) + I(2); 
    H11 = I(1) + m(2)*(l(1)^2 + l(1)*c(2)*cos(theta{2}(i))) + H12;
    H22 = I(2); 

    h = m(2)*c(2)*sin(theta{2}(i));
    G1 = m(1)*g*c(1)*cos(theta{1}(i)) + m(2)*g*( c(2)*cos(theta{1}(i) + theta{2}(i)) + l(1) * cos(theta{1}(i) ) ); 
    G2 = m(2)*g*c(2)*cos(theta{1}(i) + theta{2}(i));
    
    % Get actuator torques
    tau{1}(i) = [H11, H12] * [ddtheta{1}(i); ddtheta{2}(i)] - h*(dtheta{2}(i))^2 ...
        -2*h*dtheta{1}(i)*dtheta{2}(i) + G1;
    tau{2}(i) = [H12, H22] * [ddtheta{1}(i); ddtheta{2}(i)] + h*(dtheta{1}(i))^2 + G2;   
end
